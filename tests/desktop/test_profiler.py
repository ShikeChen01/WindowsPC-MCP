...
from windowspc_mcp.desktop.profiler import ActionProfiler, ActionTiming, InputActionType
...
class TestSetDefaultTimings:
    def test_provides_all_action_types(self, profiler: ActionProfiler) -> None:
        ...
        for action_type in InputActionType:
...
class TestCalibrate:
    def test_records_correct_timings(self, profiler: ActionProfiler) -> None:
        ...
        profiler.calibrate({InputActionType.MOVE: mock_move})

        assert call_count == 20
        timing = profiler.get_timing(InputActionType.MOVE)
        assert timing is not None
    ...
    def test_calibrate_multiple_action_types(self, profiler: ActionProfiler) -> None:
        ...
        fns = {
            InputActionType.MOVE: lambda: None,
            InputActionType.CLICK: lambda: None,
            InputActionType.KEY: lambda: None,
        }
    ...
    def test_calibrate_sets_is_calibrated(self, profiler: ActionProfiler) -> None:
        ...
        profiler.calibrate({InputActionType.MOVE: lambda: None})
...
class TestEstimate:
    def test_uses_p95(self, profiler_with_defaults: ActionProfiler) -> None:
        ...
        timing = profiler_with_defaults.get_timing(InputActionType.CLICK)
        assert timing is not None
        est = profiler_with_defaults.estimate(InputActionType.CLICK)
    ...
    def test_complexity_scales_estimate(self, profiler_with_defaults: ActionProfiler) -> None:
        ...
        base = profiler_with_defaults.estimate(InputActionType.STRING, complexity=1.0)
        scaled = profiler_with_defaults.estimate(InputActionType.STRING, complexity=3.0)
    ...
    def test_various_complexity_values(self, profiler_with_defaults: ActionProfiler) -> None:
        ...
        for factor in [0.5, 1.0, 2.0, 4.0, 10.0]:
            est = profiler_with_defaults.estimate(InputActionType.DRAG, complexity=factor)
            timing = profiler_with_defaults.get_timing(InputActionType.DRAG)
            assert timing is not None
    ...
    def test_raises_key_error_for_unknown(self, profiler: ActionProfiler) -> None:
        ...
        with pytest.raises(KeyError):
            profiler.estimate(InputActionType.MOVE)
...
class TestRecord:
    def test_creates_new_entry_for_unknown(self, profiler: ActionProfiler) -> None:
        ...
        profiler.record(InputActionType.SCROLL, 1.5)
        timing = profiler.get_timing(InputActionType.SCROLL)
        assert timing is not None
    ...
    def test_ema_shifts_mean_toward_new_value(
        ...
        old = profiler_with_defaults.get_timing(InputActionType.CLICK)
        assert old is not None
        old_mean = old.mean

        # Record a value much higher than the default mean (0.5)
        profiler_with_defaults.record(InputActionType.CLICK, 10.0)

        new = profiler_with_defaults.get_timing(InputActionType.CLICK)
    ...
    def test_ema_shifts_p95_toward_new_value_when_higher(
        ...
        profiler_with_defaults.record(InputActionType.CLICK, 50.0)
    ...
    def test_p95_does_not_decrease_on_low_value(
        ...
        profiler_with_defaults.record(InputActionType.CLICK, 0.001)
    ...
    def test_record_increments_samples(
        ...
        old = profiler_with_defaults.get_timing(InputActionType.KEY)
        assert old is not None
        old_samples = old.samples

        profiler_with_defaults.record(InputActionType.KEY, 0.2)
        profiler_with_defaults.record(InputActionType.KEY, 0.3)

        new = profiler_with_defaults.get_timing(InputActionType.KEY)
        assert new is not None
    ...
    def test_ema_formula_correctness(self, profiler: ActionProfiler) -> None:
        ...
        profiler.record(InputActionType.MOVE, 1.0)  # creates entry: mean=1.0
        profiler.record(InputActionType.MOVE, 5.0)  # EMA update
    ...
    def test_multiple_records_converge(self, profiler_with_defaults: ActionProfiler) -> None:
        ...
        target = 10.0
        for _ in range(100):
            profiler_with_defaults.record(InputActionType.MOVE, target)

        timing = profiler_with_defaults.get_timing(InputActionType.MOVE)
        assert timing is not None
...
class TestIsCalibrated:
    ...
    def test_true_after_record(self, profiler: ActionProfiler) -> None:
        profiler.record(InputActionType.CLICK, 0.5)
        assert profiler.is_calibrated
...
class TestGetTiming:
    def test_returns_none_for_unknown(self, profiler: ActionProfiler) -> None:
        assert profiler.get_timing(InputActionType.MOVE) is None
